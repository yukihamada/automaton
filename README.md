...
```
タスク受注 → 計画 → 人間の承認 → 実行 → 報酬(USDC) → 自己改善 → 繰り返し
```
...
全ての行動に「**なぜそうしたか**」の理由をテキスト出力します。問題が起きたとき「AI がどこで判断ミスをしたか」を後から完全に検証できます。

```
[思考] サーバーの応答が遅い。メモリリークの可能性がある
[計画] heap profiler を使って調査したい → 承認を要求
[承認待ち] 人間の承認を待機中...
[実行] 承認を得た。profiler を実行 → リーク箇所を特定
```
...
src/
├── agent/
│   ├── agent_loop.rs       # メイン ReAct ループ + バックグラウンドタスク
